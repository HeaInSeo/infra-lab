...
func readOnlyTools(info bootstrapInfo) map[string]toolHandler {
	...
	add("version.v1", "version", "Show infra-lab version metadata.", emptySchema(), noArgs("version"), 30*time.Second)
	add("capabilities.v1", "capabilities", "Show ilab JSON contract capabilities.", emptySchema(), noArgs("capabilities"), 30*time.Second)
	...
	add("vm.version.v1", "vm_version", "Read infra-lab build metadata and guest OS info from a VM.", vmVersionSchema(), vmVersionArgs(), 30*time.Second)
	add("profile.list.v1", "profile_list", "List available profiles.", emptySchema(), noArgs("profile", "list"), 30*time.Second)
	add("profile.show.v1", "profile_show", "Show normalized profile data.", profileSchema(), profileArgs("profile", "show"), 30*time.Second)
	add("profile.validate.v1", "profile_validate", "Validate a profile.", profileSchema(), profileArgs("profile", "validate"), 30*time.Second)
	addSynthetic([]string{"env.status.v1", "vm.list.v1", "k8s.status.v1"}, "collect_snapshot", "Collect a read-only infra-lab health snapshot.", envSchema(), syntheticToolMeta("evidence", "LOW", false, false, "Stage 2"), snapshotArgs(), 90*time.Second)
	addSynthetic([]string{"env.status.v1", "vm.list.v1", "k8s.status.v1"}, "summarize_health", "Summarize read-only infra-lab snapshot health.", envSchema(), syntheticToolMeta("evidence", "LOW", false, false, "Stage 2"), healthSummaryArgs(), 90*time.Second)
	addSynthetic([]string{"profile.validate.v1"}, "up_plan", "Create a plan-only env up proposal.", planSchema(false), syntheticToolMeta("plan", "MEDIUM", false, true, "Stage 3"), planArgs("env_up"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "down_plan", "Create a plan-only env down proposal.", planSchema(false), syntheticToolMeta("plan", "HIGH", true, true, "Stage 3"), planArgs("env_down"), 30*time.Second)
	addSynthetic([]string{"profile.validate.v1", "env.status.v1"}, "rebuild_plan", "Create a plan-only env rebuild proposal.", planSchema(false), syntheticToolMeta("plan", "HIGH", true, true, "Stage 3"), planArgs("rebuild"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "addon_install_plan", "Create a plan-only addon install proposal.", planSchema(true), syntheticToolMeta("plan", "MEDIUM", false, true, "Stage 3"), planArgs("addon_install"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "addon_uninstall_plan", "Create a plan-only addon uninstall proposal.", planSchema(true), syntheticToolMeta("plan", "HIGH", true, true, "Stage 3"), planArgs("addon_uninstall"), 30*time.Second)
	addSynthetic([]string{"profile.validate.v1"}, "profile_clone", "Clone a profile into the user profile directory without overwriting existing files.", profileWriteSchema(true), syntheticToolMeta("profile_write", "MEDIUM", false, false, "Stage 4"), profileWriteArgs("clone"), 30*time.Second)
	addSynthetic([]string{"profile.validate.v1"}, "profile_save_as", "Create a new profile in the user profile directory without touching infrastructure.", profileWriteSchema(false), syntheticToolMeta("profile_write", "MEDIUM", false, false, "Stage 4"), profileWriteArgs("save_as"), 30*time.Second)
	addSynthetic([]string{"profile.validate.v1"}, "profile_validate_and_save", "Validate and save a new profile in the user profile directory.", profileWriteSchema(false), syntheticToolMeta("profile_write", "MEDIUM", false, false, "Stage 4"), profileWriteArgs("validate_and_save"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "addon_install_prepare", "Prepare an approved addon install operation.", addonPrepareSchema(), syntheticToolMeta("approved_mutation", "MEDIUM", false, true, "Stage 5"), addonPrepareArgs(), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "addon_install_commit", "Commit a prepared addon install operation after approval.", addonCommitSchema(), syntheticToolMeta("approved_mutation", "MEDIUM", false, true, "Stage 5"), addonCommitArgs(), 15*time.Minute)
	addSynthetic([]string{"env.status.v1"}, "operation_status", "Read an infra-lab operation status record.", operationSchema(), syntheticToolMeta("operation", "LOW", false, false, "Stage 5"), operationArgs("status"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "operation_logs", "Read stdout/stderr logs for an infra-lab operation.", operationSchema(), syntheticToolMeta("operation", "LOW", false, false, "Stage 5"), operationArgs("logs"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "operation_approve", "Approve a prepared infra-lab operation before commit.", operationSchema(), syntheticToolMeta("operation", "MEDIUM", false, false, "Stage 5"), operationArgs("approve"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "operation_cancel", "Cancel a prepared or approved infra-lab operation before it runs.", operationSchema(), syntheticToolMeta("operation", "MEDIUM", false, false, "Stage 5"), operationArgs("cancel"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "operation_locks", "List current infra-lab env locks.", emptySchema(), syntheticToolMeta("operation", "LOW", false, false, "Stage 5"), noArgs("__operation__", "locks"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "operation_unlock_stale", "Remove an expired env lock only when it is stale.", operationUnlockSchema(), syntheticToolMeta("operation", "MEDIUM", false, false, "Stage 5"), operationUnlockArgs(), 30*time.Second)
	addSynthetic([]string{"profile.validate.v1", "env.status.v1"}, "env_up_prepare", "Prepare an approved new environment creation operation.", envUpPrepareSchema(), syntheticToolMeta("approved_env_up", "HIGH", false, true, "Stage 6"), envUpPrepareArgs(), 30*time.Second)
	addSynthetic([]string{"profile.validate.v1", "env.status.v1"}, "env_up_commit", "Commit a prepared new environment creation operation after approval.", addonCommitSchema(), syntheticToolMeta("approved_env_up", "HIGH", false, true, "Stage 6"), envUpCommitArgs(), 30*time.Minute)
	addSynthetic([]string{"env.status.v1"}, "env_down_prepare", "Prepare an approved destructive env down operation.", destructivePrepareSchema(false, false), syntheticToolMeta("destructive_execution", "HIGH", true, true, "Stage 7"), destructivePrepareArgs("env_down_prepare"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "env_down_commit", "Commit a prepared env down operation after approval.", addonCommitSchema(), syntheticToolMeta("destructive_execution", "HIGH", true, true, "Stage 7"), destructiveCommitArgs("env_down_commit"), 30*time.Minute)
	addSynthetic([]string{"env.status.v1"}, "env_clean_prepare", "Prepare an approved destructive env clean operation.", destructivePrepareSchema(false, false), syntheticToolMeta("destructive_execution", "HIGH", true, true, "Stage 7"), destructivePrepareArgs("env_clean_prepare"), 30*time.Second)
	addSynthetic([]string{"env.status.v1"}, "env_clean_commit", "Commit a prepared env clean operation after approval.", addonCommitSchema(), syntheticToolMeta("destructive_execution", "HIGH", true, true, "Stage 7"), destructiveCommitArgs("env_clean_commit"), 10*time.Minute)
	addSynthetic([]string{"profile.validate.v1", "env.status.v1"}, "env_rebuild_prepare", "Prepare an approved destructive env rebuild operation.", destructivePrepareSchema(true, false), syntheticToolMeta("destructive_execution", "HIGH", true, true, "Stage 7"), destructivePrepareArgs("env_rebuild_prepare"), 30*time.Second)
